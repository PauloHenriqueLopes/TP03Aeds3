...
public class ListaInvertida {
  ...
  class Bloco {
    ...
    public byte[] toByteArray() throws IOException {
      ...
      while (i < quantidade) {
        ...
        dos.writeFloat(elementos[i].getTfidf());
        i++;
      }
  ...
  public float calculateIdf(String termo) throws Exception {
    // Conta o número total de documentos (entidades)
    int totalDocumentos = numeroEntidades();
    if (totalDocumentos == 0) return 0;
    
    // Conta em quantos documentos o termo aparece
    ElementoLista[] elementos = read(termo);
    int documentosComTermo = (elementos != null) ? elementos.length : 0;
    if (documentosComTermo == 0) return 0;
    
    // Calcula o IDF
    return (float) Math.log10((float) totalDocumentos / documentosComTermo);
}

public void updateTfidf(String termo) throws Exception {
    float idf = calculateIdf(termo);
    ElementoLista[] elementos = read(termo);
    
    if (elementos != null) {
        for (ElementoLista el : elementos) {
            // Atualiza o TF-IDF (TF já está armazenado, multiplica pelo IDF)
            el.setTfidf(el.getTfidf() * idf);
            update(termo, el);
        }
    }
}
